...
外围 292 棵树使用 `parkTreeLod.js` 按 96 米网格分成 23 区，每区两组实例分别绘制树干和树冠。2026-09-17 按用户要求统一为与园内一致的三团实心卡通树冠，树的位置、朝向、尺度保留；树冠从 `#94b486` 到 `#b2c99a` 做实例颜色变化，树干统一为 `#9b917a`。去除透明叶卡、树皮与叶片纹理，保留廊架木纹。园内、道路及门岗现有树木已经采用同类几何，因此无需重建 GLB。

三档几何每棵分别为 312 / 116 / 76 个三角面（此前为 436 / 216 / 100）；近景树冠使用三组细分二十面体，中远景减少细分及枝干。各档树冠整体尺寸一致，切档仍会有正常的棱角变化。各区沿用全部档位的包围球并集，避免切档导致边缘提前消失。
...
此前叶卡版本以原首页相机、42° 视角、1440×900 画幅做相机剔除统计，树木候选三角面从 127,312 降至约 38,608（约 70%）；树木绘制批次从 2 增至约 30。统计包含分区整体剔除，不是 GPU 时间或帧率实测，也不代表本次卡通树的实测结果。本次未修改自动环绕、画质分辨率或帧率策略。

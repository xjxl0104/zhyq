...
**水表拓扑（负责人 2026-09-12 14:30 补充，只记文档、暂不在系统里算）**：大厦水表装在**锦华工业园**那边，是对外计费的发票口径；园区水表 SB-4475 量的是真正进园区的水，**用来算水损**：水损 = 大厦总表 − 园区总表（8 月 1012 − 710 = 302）。租户公摊仍按大厦总表算（1012 − 562.5 = 449.5），管网水损目前摊在公摊里。负责人拍板暂不在系统显示水损；以后要算，口径就是 MAIN − REFERENCE，代码入口 `EnergyStatsController.combine`。

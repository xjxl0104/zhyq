...
> 本系统按需求**不含登录鉴权页与视频编解码流**;RBAC 数据模型(用户/角色/部门/岗位/权限)已建库表并可维护。

## 技术栈

| 层 | 选型 |
|---|---|
| 后端 | Spring Boot 3.2、MyBatis-Plus 3.5、MySQL 8、Flyway、Knife4j(接口文档)、Hutool |
| 前端 | Vue 3、Vite 5、Element Plus、Pinia、Vue Router、ECharts 5 |
...
│       └── resources/db/migration/  # Flyway V1~V8(建表 + 种子数据)
├── frontend/                   # Vue 3 前端
│   └── src/
│       ├── layout/             # 14 个一级导航布局
...
当前后台接口未加鉴权层(按需求跳过登录)。**若投入真实环境,必须在网关或 Spring Security 层补充认证授权**,并对高风险操作(删除/退款/调账/设备控制)加二次确认与审计,详见需求规格书 §20.2。

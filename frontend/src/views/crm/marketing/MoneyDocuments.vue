<template>
  <div class="page-container">
    ...
    <div class="table-card">
      <div class="toolbar"><el-button v-if="canEdit" type="primary" @click="openGenerate">{{ isBill ? '生成服务费账单' : '生成云仓结算' }}</el-button><el-button v-if="isBill && canGenerate" :loading="generatingFixed" @click="generateFixed">补齐到期固定月费</el-button><el-button v-if="isBill && canPay" @click="openDirect">登记直签平台费到账</el-button><el-button v-if="isBill" @click="showDirectRecords">平台费到账记录</el-button></div>
    ...
    <el-dialog v-model="generation.visible" :title="isBill ? '生成服务费账单' : '生成云仓结算'" width="min(540px, 94vw)">
      <el-form label-position="top">
        ...
        <p class="hint">{{ isBill ? '仅汇总本合同已确认的出库单；重复或重叠账期不会再次计费。' : '只使用已实际收款且未结算的账单，金额按云仓应付费率计算并固定保存。' }}</p>
      </el-form>
    ...
    <el-dialog v-model="direct.visible" title="登记直签平台费到账" width="min(580px, 94vw)">
      <el-form label-position="top">
        <el-form-item label="已生效的云仓直签合同" required><el-select v-model="direct.contractId" filterable class="full" @change="direct.files=[]"><el-option v-for="c in directContracts" :key="c.id" :label="`${c.contractNo} · ${warehouseName(c.warehouseId)}`" :value="c.id" /></el-select></el-form-item>
...
const warehouses=ref([]),contracts=ref([]),parkContracts=computed(()=>contracts.value.filter(c=>c.signMode===1&&[4,5,6,7].includes(c.status))),directContracts=computed(()=>contracts.value.filter(c=>c.signMode===2&&[4,5,6].includes(c.status)))
...
const generation=reactive({visible:false,targetId:null,period:[]}),payment=reactive({visible:false,row:null,number:'',files:[]}),direct=reactive({visible:false,contractId:null,amount:'',paymentNo:'',files:[]})
async function loadChoices(){const [w,c]=await Promise.all([mktWarehouseApi.page({pageNo:1,pageSize:100}),mktContractApi.page({pageNo:1,pageSize:100})]);warehouses.value=w.records||[];contracts.value=c.records||[]}
async function openGenerate(){await loadChoices();Object.assign(generation,{visible:true,targetId:null,period:[]})}
...
async function openDirect(){await loadChoices();Object.assign(direct,{visible:true,contractId:null,amount:'',paymentNo:'',files:[]})}
async function recordDirect(){if(!direct.contractId||!/^\d+(\.\d{1,2})?$/.test(direct.amount)||Number(direct.amount)<=0||!direct.paymentNo.trim()||!direct.files.length)return ElMessage.warning('请选择生效合同，填写正数金额、流水号并上传凭证');saving.value=true;try{const c=directContracts.value.find(c=>c.id===direct.contractId);await mktSettlementApi.directPayment({contractId:c.id,warehouseId:c.warehouseId,paymentNo:direct.paymentNo.trim(),amount:direct.amount,payProof:`file:${direct.files[0].id}`});direct.visible=false;ElMessage.success('平台费已登记，佣金已生成')}finally{saving.value=false}}
...
onMounted(()=>{load();mktWarehouseApi.page({pageNo:1,pageSize:100}).then(r=>warehouses.value=r.records||[]).catch(()=>{})})
</script>
<style scoped>

      ...
      <template v-if="!moduleView">
        ...
        <div class="twin-content-grid" :class="{ 'is-focused': modelFocus }">
          ...
          <section class="twin-viewport" aria-label="三维空间工作台">
            <div class="viewport-top"><div class="scene-heading"><span class="scene-heading-dot" /><strong>{{ mode === 'interior' ? (floor || 3) + 'F · 仓内空间' : viewpoint === 'entrance' ? '园区主入口' : '园区鸟瞰' }}</strong><span>3D VIEW</span></div><div class="view-switch" aria-label="模型视图"><button v-for="view in [{ id: 'exterior', icon: 'cube', name: '建筑外观' }, { id: 'exploded', icon: 'layers', name: '楼层展开' }, { id: 'interior', icon: 'eye', name: '室内空间' }]" :key="view.id" :data-testid="'view-' + view.id" :class="{ active: mode === view.id }" :aria-pressed="mode === view.id" @click="setMode(view.id)"><TwinIcon :name="view.icon" :size="15" /><span>{{ view.name }}</span></button></div></div>
            <button class="model-focus-toggle" :aria-label="modelFocus ? '恢复运营看板' : '专注查看模型'" :aria-pressed="modelFocus" @click="toggleModelFocus"><TwinIcon :name="modelFocus ? 'grid' : 'expand'" :size="13" />{{ modelFocus ? '返回看板' : '专注模型' }}</button>

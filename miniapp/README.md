...
- 账号和密码只需填写，不要求最低长度或字符组合，支持 `123 / 123`、中文及符号。账号沿用去除首尾空格、统一小写及同身份唯一规则，数据库存储容量为 512 个字符，超出时明确报错。密码完整参与 SHA-256 预哈希再 BCrypt 加盐散列，不设业务长度上限，不会在 72 字节处截断；已有 BCrypt 密码仍可登录和修改。
